...
function get_version() {
  ...
  if [[ ${VERSION} =~ "gpdb" ]]; then
    ...
    if [ "${quicklz_test}" -eq "1" ]; then
      ...
      SMALL_STORAGE="appendonly=true, orientation=column, compresstype=zstd, blocksize=1048576"
      MEDIUM_STORAGE="appendonly=true, orientation=column, compresstype=zstd, blocksize=1048576"
      LARGE_STORAGE="appendonly=true, orientation=column, compresstype=zstd, compresslevel=1, blocksize=1048576"
    fi
  else
    SMALL_STORAGE=""
    MEDIUM_STORAGE=""
